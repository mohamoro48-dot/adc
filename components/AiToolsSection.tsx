...
import { BulbIcon, RobotIcon, ChatIcon, ChartIcon, RocketIcon } from './icons';
import { SIMULATED_PLANS, SIMULATED_REPORT, SIMULATED_QNA } from '../constants';
import { generateMarketingContent } from '../services/geminiService';
import type { PlanData, MarketingAngle } from '../types';
...
const AiToolsSection: React.FC<AiToolsSectionProps> = ({ id }) => {
    ...
    return (
        <CollapsibleSection id={id} title="2. المنصة الرقمية: أدوات الذكاء الاصطناعي" icon={<BulbIcon />} borderColorClass="border-trust-blue">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="space-y-6">
                    ...
                    <AiToolCard title="مُنشئ خطط العمل الفردية السريع" description="يولد خطط فردية قابلة للتنفيذ في ثوانٍ. دور الأخصائي: مراجعة الخطة المولدة، تعديلها، وتخصيصها لتناسب حالة الطالب بدقة." icon={<RobotIcon />} borderColor="border-highlight-cyan/50">
                        ...
                        {selectedPlan && (
                            <div className="p-4 bg-white rounded-lg border border-gray-300 text-sm animate-fade-in">
                                ...
                                <pre className="whitespace-pre-wrap font-sans text-gray-800 text-xs">{selectedPlan.content}</pre>
                            </div>
                        )}
                    </AiToolCard>
                    <AiToolCard title="مستشار الدعم السلوكي الفوري" description="يقدم إرشادات سريعة ومصاغة باحترافية للرد الفوري على أسئلة الولي. دور الأخصائي: يستخدم الرد كمسودة ويصيغ الرد النهائي بلمسة شخصية." icon={<ChatIcon />} borderColor="border-highlight-cyan/50">
                        ...
                        {showQnA && (
                            <div className="space-y-2 animate-fade-in">
                                ...
                                <div className="mt-2 p-4 bg-white rounded-lg border border-gray-300 text-sm">
                                    ...
                                    <pre className="whitespace-pre-wrap font-sans text-gray-800 text-xs">{SIMULATED_QNA.answer}</pre>
                                </div>
                            </div>
                        )}
                    </AiToolCard>
                ...
                <div className="space-y-6">
                    ...
                    <AiToolCard title="مُلخص التقارير الدورية الآلي" description="يلخص التقارير المعقدة إلى ملخصات موجزة وإجراءات منزلية. دور الأخصائي: يراجع الملخص الآلي للتأكد من دقته ثم يرسله لولي الأمر." icon={<ChartIcon />} borderColor="border-highlight-emerald/50">
                        ...
                        {showReport && (
                            <div className="space-y-2 animate-fade-in">
                                <div className="p-4 bg-gray-100 rounded-lg border border-gray-300 text-xs">
                                    ...
                                    <pre className="whitespace-pre-wrap font-sans text-gray-800">{SIMULATED_REPORT.input}</pre>
                                </div>
                                <div className="mt-2 p-4 bg-white rounded-lg border border-gray-300 text-xs">
                                    ...
                                    <pre className="whitespace-pre-wrap font-sans text-gray-800">{SIMULATED_REPORT.summary}</pre>
                                    <pre className="whitespace-pre-wrap font-sans text-gray-800 mt-2 border-t pt-2">{SIMULATED_REPORT.recommendations}</pre>
                                </div>
                            </div>
                        )}
                    </AiToolCard>
                     <AiToolCard title="مُولِّد الأفكار التسويقية" description="أداة تسويقية لتوليد أفكار وشعارات جديدة ومناسبة لتحسين محركات البحث (SEO) بناءً على مفاهيم مبتكرة." icon={<RocketIcon />} borderColor="border-accent-amber/50">
